...
public class TrainerScheduleDetailsInsertServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("99999999999++++++"+trainerId);
		...
		//trainerScheduleDetailsBean.setTrainerScheduleId(scheduleId);
		...
		/*if (trainerId.equals("0")) 
		{
			...
			request.setAttribute("msgTrainerName",
					"<font color=red>* TrainerId is Required</font>");
		}else{
		...
		if (isError) 
		{
			...
			request.getRequestDispatcher("trainerScheduleDetailsInsert.jsp").forward(request, response);
		} 
		else 
		{
		//	trainerScheduleDetailsBean.setTrainerScheduleId(scheduleId);
			...
			if(new TrainerScheduleDetailsDAO().insert(trainerScheduleDetailsBean)) {
				  ...
				  response.sendRedirect("TrainerScheduleDetailsListServlet?trainerId="+trainerScheduleDetailsBean.getTrainerId());
