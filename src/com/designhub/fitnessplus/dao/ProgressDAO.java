...
public List<MemberProgressBean> list(){
		...
		public boolean update(MemberProgressDetailsBean progressbean)
		{
			conn=DBConnection.getConnection();
			try {
		...
		public boolean delete(String progressDetailId) {

			conn=DBConnection.getConnection();
			String deleteSQL="delete from memberprogressdetails where progressDetailID = "+progressDetailId;
			try {
				pstmt=conn.prepareStatement(deleteSQL);
				rowsAffected=pstmt.executeUpdate();
				if(rowsAffected>0)
